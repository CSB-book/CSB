...
# To execute the commands, press CTRL+Return/Enter for each line
...
# scatterplots
pl <- ggplot(data = popsize %>% filter(Herd == "WAH")) + aes(x = Year, y = Pop_Size) + geom_point()
...
pl <- ggplot(data = popsize, aes(x = Herd, y = Pop_Size, fill = Herd)) + geom_boxplot()
show(pl)
...
pl <- ggplot(data = seaice %>% filter(Herd == "BEV")) + aes(x = Year, y = Month, colour = Cover, size = Cover) + geom_point()
show(pl)
...
ggplot(data = seaice %>% filter(Herd %in% c("WAH", "BAT"), Year %in% c(1980, 1990, 2000, 2010))) + aes(x = Month, y = Cover) + geom_col() + facet_grid(Year~Herd)

# facetting, ommit missing data
ggplot(data = seaice %>% filter(Year == 2010)) + aes(x = Month, y = Cover) + geom_col() + facet_wrap(~Herd)
...
# legend guides
pl <- ggplot(data = popsize) + aes(x = Year, y = Pop_Size, colour = Herd, alpha = sqrt(Pop_Size)) + geom_point()
show(pl)
